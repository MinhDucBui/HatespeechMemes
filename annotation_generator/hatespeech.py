...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_folder', '-o', type=str,
                        default='/Users/duc/Desktop/Projects/Ongoing/MultiModalMemes/dataset/annotation/hatespeech')
    ...
    for language in LANGUAGES:
        ...
        # Load the text file into a DataFrame
        final_file = os.path.join(final_dataset, language + "_translation.xlsx")
        ...
        #df_annotation = df_annotation[df_annotation['ID'].isin([56603606])]
        ...
        column_names = ['ID', 'Image', 'Caption', 'Hate Speech (=1) or Non-Hate Speech (=0)', 'Comments']  # Replace with your desired column names
        ...
        df_annotation.loc[non_empty, 'Translation'] = df_annotation.loc[non_empty, 'Correct Translation']
        df_hatespeech["Caption"] = df_annotation["Translation"]

        writer = pd.ExcelWriter(os.path.join(output_folder, language + ".xlsx"), engine='xlsxwriter')
        ...
        col_idx = df_hatespeech.columns.get_loc('Hate Speech (=1) or Non-Hate Speech (=0)') + 1
        ...
        for index, instance in tqdm(enumerate(instances_processing)):
            ...
            caption = df_hatespeech[df_hatespeech["ID"] == instance]["Caption"].iloc[0]
            ...
            # Get Image Path
            template_name = df_caption[df_caption["instance_id"] == row["instance_id"]]["template"].iloc[0]
            ...
            make_meme(top, bottom, image_file, output_path, worksheet, index+2, language=language)
